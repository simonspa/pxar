...
void ConfigParameters::readRocPixelConfig() {
  ...
  for (unsigned int i = 0; i < fnRocs; ++i) {
    ...
    int i2c;
    if (fI2cAddresses.size()>i)
    	i2c = fI2cAddresses[i];
    else
    	i2c = i;
    fname << fDirectory << "/" << fTrimParametersFileName << fTrimVcalSuffix << "_C" << i2c << ".dat";
...
void ConfigParameters::readRocDacs() {
  if (!fReadDacParameters) {
    for (unsigned int i = 0; i < fnRocs; ++i) {
      ...
      filename << fDirectory << "/" << fDACParametersFileName << fTrimVcalSuffix << "_C" << i2c << ".dat";
...
void ConfigParameters::readNrocs(string line) {
	...
	if (string::npos == s1) {
		...
		for (unsigned int roc = 0; roc < fnRocs; roc++){

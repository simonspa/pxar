...
void PxarSatellite::initializing(Configuration& config) {
    ...
    try {
        ...
        // test pixels
        if(testpulses) {
            LOG(INFO) << "Setting up pixels for calibrate pulses..." << std::endl << "col \t row" << std::endl;
            for(int i = 40; i < 45; i++) {
...
void PxarSatellite::starting(std::string_view /*run_identifier*/) {
    ...
    try {
        ...
        LOG(INFO) << "BOR message with detector " << m_detector << " (event type " << m_event_type << ") and ROC type "
                  << m_roctype;
    } catch(pxar::pxarException& e) {
        throw SatelliteError(std::string("pxar exception: ") + e.what());
    }
...
void PxarSatellite::running(const std::stop_token& stop_token) {
    ...
    while(!stop_token.stop_requested()) {
        ...
        // Trying to get the next event, daqGetRawEvent throws exception if none is
        // available:
        try {
...
std::vector<std::pair<std::string, uint8_t>> PxarSatellite::GetConfDACs(Configuration& config, int16_t i2c, bool tbm) {
    ...
    if(!file.fail()) {
        ...
        while(std::getline(file, line)) {
            ...
            if(name.empty()) {
                ...
                throw pxar::InvalidConfig("WARNING: Problem reading DACs from file \"" + filename +
...
std::vector<pxar::pixelConfig> PxarSatellite::GetConfTrimming(Configuration& config,
                                                              std::vector<pxar::pixelConfig> maskbits,
                                                              int16_t i2c) {

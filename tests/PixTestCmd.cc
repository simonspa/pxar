...
void CmdProc::setApi(pxar::pxarCore * api, PixSetup * setup){
    ...
    fnTbmCore =   fApi->_dut->getNTbmCores();
...
int CmdProc::test_timing2(int nloop, int d160, int d400, int rocdelay[], int htdelay[], int tokdelay[],
    ...
    int nerr = countGood(nloop, fNtrigTimingTest, ftrigkhz, nroc);
...
//int CmdProc::find_2tbm_timing() {
//    size_t n_cores = fApi->_dut->getNTbmCores();
//    for (uint8_t i_core = 0; i_core < n_cores; i_core++){
//        vector<pair<string, uint8_t> > regs = fApi->_dut->getTbmDACs(i_core);
//        cout << "Core: " << int(i_core) << endl;
//        for (vector<pair<string, uint8_t> >::iterator it_reg = regs.begin(); it_reg != regs.end(); it_reg++){
//            bitset<8> val(it_reg->second);
//            cout << "  " << it_reg->first << " " << val << hex << " 0x" << int(it_reg->second) << endl;
//        }
//    }
//    // disable token pass
//    tbmsetbit("base0", 15, 6, 1);
//    // diagonal scan
//    for (uint8_t m=0; m<8; m++) {
//        uint8_t value = ((m & 0x7) << 5) + ((m & 0x7) << 2);
//        bitset<8> val(value);
//        cout << "diagonal scan for basee: " << val << endl;
//        if (tbmset("basee", 5, value)) cout << "set basee failed! " << value << endl;
//        cout << countGood(10, 100, 100, 16) << endl;
//    }
//    return 2;
//
//}
...
TBMDelays CmdProc::tbmgetDelays(){
    // get all delay settings into a TBMDelays object
    TBMDelays d;
    for(uint8_t core=0; core<fnTbmCore; core++){
       if (core%2==0){
            tbmget("basee", 1<<core, d.register_e[core]);
            d.d400= (d.register_e[core] >> 2) & 0x7;
            d.d160= (d.register_e[core] >> 5) & 0x7;
        }
        tbmget("base0", 1<<core, d.register_0[core]);
        tbmget("basea", 1<<core, d.register_a[core]);
...
int CmdProc::post_timing(){
    ...
    for(uint8_t port=0; port<2*fnTbmCore; port++){
        ...
        uint8_t portmax= 0;
        int delay = t.rocdelay[port]; // save
        for(uint8_t dport=0; dport<8; dport++){
            int daqchannel = daqChannelFromTbmPort( port ); 
            t.rocdelay[ port ] = dport;
            test[dport] = test_timing2(nloop, t.d160, t.d400, t.rocdelay, t.htdelay, t.tokendelay, daqchannel);
            if (test[dport]>test[portmax]) portmax=dport;
        }
    ...
    set_tbmtiming( t.d160, t.d400, t.rocdelay, t.htdelay, t.tokendelay);
    fBufsize = bufsize;
        ...
        if (data.size()<nSample) {
            ...
            for(unsigned int istart=0; istart<b.size()-30; istart++){
			   ...
			   bool valid = (S==(regId | 1))  && (H==hubId) && (P==4)
				&& bstop && !compS3 && !compRW && !compD4 && !compD0;
					
               /* 
                if( (S==(regId | 1))  && (H==hubId) && (P==4) ){
                    cout << "tbmread " << dly  <<  " " << istart << "  valid=" << valid 
...
int CmdProc::rawRocReadback(uint8_t  signal, std::vector<uint16_t> & values){
    ...
    size_t nTBM = fApi->_dut->getNTbmCores();
...
int CmdProc::setupDaq(int ntrig, int ftrigkhz, int verbosity){
    ...
    if((ntrig>0) && (fApi->_dut->getNTbmCores() > 0) && ((fSeq & 0x02 ) == 0 ) ){
        out << "The current sequence does not contain a trigger!\n";
    }else if((fApi->_dut->getNTbmCores() == 0) && ((fSeq & 0x01 ) == 0 ) ){
...
int CmdProc::burst(vector<uint16_t> & buf, int ntrig, int trigsep, int nburst, int caltrig, int verbosity){
    ...
    // warn user when no data expected
    if((fApi->_dut->getNTbmCores() > 0) && ((fSeq & 0x02 ) == 0 ) ){
        out << "The current sequence does not contain a trigger!\n";
    }else if((fApi->_dut->getNTbmCores() == 0) && ((fSeq & 0x01 ) == 0 ) ){
        out <<"The current sequence does not contain a token!\n";
    }
...
int CmdProc::getData(vector<uint16_t> & buf, vector<DRecord > & data, int verbosity, 
    ...
    else if (fApi->_dut->getNTbmCores() == 0) {
        // single ROC
    ...
    if (level>0){
        ...
        for(unsigned int i=0; (i<data.size())&&(lines<maxLines); i++){
            ...
            if (data[i].type==4){
                ...
                if((tbm==0)&&(fApi->_dut->getNTbmCores() > 0)) out << ", Warning! TBM header expected first!";
                out << "\n";
                roc ++;
                lines++;
        ...
        if((nRocHeader>0)&&(fApi->_dut->getNTbmCores() > 0)){
...
bool CmdProc::process(Keyword keyword, Target target, bool forceTarget, int index){
    ...
    if (keyword.match("configure")){
        ...
        for(unsigned int core=0; core<fApi->_dut->getNTbmCores(); core++){
            std::vector< std::pair<std::string,uint8_t> > regs = fApi->_dut->getTbmDACs(core);
            for(unsigned int i=0; i<regs.size(); i++){
    ...
    cout << "still nothing happening..." << endl;
    int stat  =0;
...
int CmdProc::exec(std::string s){
    ...
    for (uint8_t i = 0; i < stmts.size(); i++) cout << stmts.at(i)->keyword.str() << " " << stmts.at(i)->out_filename << endl;
    bool ok = true;
    for( unsigned int i=0; i<stmts.size(); i++){
